    ...
    if (typeof isServiceDateAvailable === "function") {
      if (isServiceDateAvailable(dateValue, serviceName)) {
        ...
        if (selectedType === "single") {
          messageDiv.textContent = "Date disponible. Veuillez procéder au paiement pour réserver cette date."; // Acompte pour séance ponctuelle
          paypalButtonSingleDiv.style.display = "block";
        } else { // "guidance"
          messageDiv.textContent = "Date disponible. Veuillez procéder au paiement pour réserver cette date."; // Acompte pour guidance complète (supposé, basé sur le script original)
